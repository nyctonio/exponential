...
function Toggle({
  ...
  const { formActive, updateData, data } = userUserAccessStore();
...
function HeaderToggle({
  ...
  const { formActive, updateData, updateTableAccess } = userUserAccessStore();
  const disabled = !formActive;
  return (
    <ToggleAntd
      checked={selected}
...
function AccessTable(data: ParsedUserAccessItem) {
  ...
  const { updateData, editedFunctions } = userUserAccessStore();
...
function UserAccessTables({
  ...
  const submitHandler = async () => {
    ...
    let res = await apiCall(
      Routes.UPDATE_USER_ACCESS_MANAGEMENT,
      { username: selectedUser?.username, editedFunctions: editedFunctions },
      false
    );
