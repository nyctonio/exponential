...
import { PlSharingType } from '@/store/advance-settings/pl-brokerage-sharing';
...
const ScriptTable = () => {
  ...
  const checkDisabled = (label: string, id: number | string) => {
    if (!formActive) {
      ...
      let check = false;
      tableData.plSharing.forEach((element) => {
        if (element.id == id) {
          // @ts-ignore
  ...
  const columns: ColumnsType<PlSharingType> = [
    {
          ...
          <div className="px-2">
            <BorderInput
              ...
              onChange={(e) => {
                setTableData({
                  ...tableData,
                  plSharing: tableData.plSharing.map((item) => {
                    if (item.id == row.id) {
                      return {
                        ...item,
    ...
    <Table
      ...
      columns={columns.filter((column) => {
        if (
          ...
          if (tableData.plSharing.length != 0) {
            if (tableData.plSharing[0].thirdparty == null) {
              return false;
            } else {
              return true;
            }
      ...
      dataSource={tableData.plSharing}
